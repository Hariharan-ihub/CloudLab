...
const FakeVPCConsole = ({ activeLab }) => {
  const dispatch = useDispatch();
  const { resources, currentStepId, completedSteps } = useSelector(state => state.simulation);
  const { user } = useSelector(state => state.auth);
  const userId = user?.id;
  const labId = activeLab?.labId || 'adhoc';
  ...
  useEffect(() => {
    if (!userId) return;
    if (activeTab === 'vpcs') dispatch(fetchResources({ userId, type: 'VPC', labId }));
  ...
  }, [dispatch, activeTab, userId, labId]);

  // Auto-validate navigation step (like EC2 does for URL_CONTAINS)
  useEffect(() => {
    if (activeLab?.steps) {
      const navStep = activeLab.steps.find(
        s => s.validationLogic?.type === 'URL_CONTAINS' && s.validationLogic.value === '/service/vpc'
      );
      if (navStep && !completedSteps.includes(navStep.stepId)) {
        handleAction(navStep.stepId, 'NAVIGATE', { url: '/service/vpc' });
      }
    }
  }, [activeLab, completedSteps]);

  // Core action dispatcher — mirrors EC2's handleAction pattern
  const handleAction = async (stepId, action, payload) => {
    try {
      const result = await dispatch(validateStep({ userId, labId, stepId, action, payload }));
  ...
  // Find the step that expects a given action + resource type
  const findStep = (actionType, resourceType) => {
    if (!activeLab?.steps) return null;
    return activeLab.steps.find(s =>
      s.expectedAction === actionType &&
      (!resourceType || s.validationLogic?.resourceType === resourceType)
    ) || null;
  };

  const handleCreate = async () => {
    let action, type, payload, resourceType;

    if (activeTab === 'vpcs') {
      action = 'CREATE_VPC'; type = 'VPC'; resourceType = 'VPC';
      payload = { name: modalData.name, cidrBlock: modalData.cidr };
    } else if (activeTab === 'subnets') {
      action = 'CREATE_SUBNET'; type = 'SUBNET'; resourceType = 'SUBNET';
      payload = { name: modalData.name, cidrBlock: modalData.cidr, vpcId: modalData.vpcId, az: modalData.az };
    } else if (activeTab === 'sgs') {
      action = 'CREATE_SECURITY_GROUP'; type = 'SECURITY_GROUP'; resourceType = 'SECURITY_GROUP';
      payload = { groupName: modalData.name, description: modalData.description, vpcId: modalData.vpcId };
    }

    const step = findStep(action, resourceType);
    const result = await handleAction(step?.stepId || currentStepId, action, payload);
  ...
  const handleDelete = async () => {
    ...
    await handleAction(null, action, { resourceId: itemToDelete._id });
      ...
      <tbody>
        {vpcs.map(vpc => (
          <tr key={vpc._id} className="border-b hover:bg-gray-50">
            <td className="px-4 py-3 font-medium text-aws-blue">{vpc.state.name}</td>
            ...
            <td className="px-4 py-3">
              <button onClick={() => { setItemToDelete(vpc); setConfirmDeleteOpen(true); }} className="text-red-600 hover:text-red-900">
                <Trash2 size={16} />
              </button>
            </td>
          </tr>
        ...
        {subnets.map(s => {
          ...
          return (
            <tr key={s._id} className="border-b hover:bg-gray-50">
              <td className="px-4 py-3 font-medium text-aws-blue">{s.state.name}</td>
              ...
              <td className="px-4 py-3">
                <button onClick={() => { setItemToDelete(s); setConfirmDeleteOpen(true); }} className="text-red-600 hover:text-red-900">
                  <Trash2 size={16} />
                </button>
              </td>
            </tr>
      ...
      <tbody>
        {securityGroups.map(sg => (
          <tr key={sg._id} className="border-b hover:bg-gray-50">
            <td className="px-4 py-3 font-medium text-aws-blue">{sg.state.groupName}</td>
            ...
            <td className="px-4 py-3">
              <button onClick={() => { setItemToDelete(sg); setConfirmDeleteOpen(true); }} className="text-red-600 hover:text-red-900">
                <Trash2 size={16} />
              </button>
            </td>
          </tr>
